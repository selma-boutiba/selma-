...
%%% WITHDRAWN --- DO NOT USE. The explanation that stood here attributed the
%%% feature-error floor (4.35e7 / 1.04e8) to "a property of the feature", i.e.
%%% to the stacked Hermite responses retaining a non-zero difference at the
%%% attained pose. That explanation is WRONG. The three source files that
%%% produced these numbers build the REFERENCE image from the OCCLUDED texture
%%% (sim.init(Itextured, X) before cdMo) and the SERVOING images from the CLEAN
%%% texture (sim.init(Itexture, X) before cMo). I* therefore contained the black
%%% ellipse and I did not, so a large part of the residual could never be
%%% cancelled by any pose. The floor is an artifact of the wiring, not a
%%% property of the feature. The nominal run must be repeated with the two
%%% sim.init calls swapped before anything is written about the residual floor.
First, the feature-error norm does not decay to zero but settles on a floor
%%% RE-MEASURE after the wiring fix.
(value to be re-measured), while the commanded velocity falls to between
$1.4\times10^{-4}$ and $3.7\times10^{-4}$ and the pose error becomes
sub-millimetre. The absolute threshold on $\norm{\mathbf{e}}$ inherited from the
...
\begin{table}[t]
    ...
    %%% INVALID DATA --- the runs behind these numbers used an occluded
    %%% reference image (see the withdrawn paragraph above). Re-run all three
    %%% variants with sim.init(Itexture, X) before cdMo and
    %%% sim.init(Itextured, X) before cMo, then refill this table. The layout,
    %%% units and headers below are correct and can be reused as-is.
    \small
